...
export default class CalendarDays extends React.Component {
  ...
  scroll = (direction) => {
    if (this.scrollView) {
      ...
      let newPosition = 0;
      if (direction === 'left') {
        newPosition = Math.max(scrollPosition - constants.DAY_SIZE - 230, 0);
      } else {
        newPosition = scrollPosition + constants.DAY_SIZE + 230;
      }
      this.setState({
            ...
            <View style={[style.singleContainer, Platform.OS !== 'web' ? selectedStyle : null]}>
              <View style={[style.singleDateBox, selectedStyle]}>
                <View style={[style.monthContainer, isClosedMonthStyle]}>
                  <Text style={style.monthText}>{moment(val.date).format('DD-MM-YYYY')}</Text>
                </View>
              </View>
            </View>
          </TouchableOpacity>
